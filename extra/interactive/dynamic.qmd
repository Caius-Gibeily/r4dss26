...
Write an R function that takes a single variable `x` and returns that value multiplied by \${rand_int}.
...
The normal distribution has 2 parameters, the mean $\mu$ and the standard deviation $\sigma$. For a normal distribution with mean $\mu =$ `{ojs} meanTgl` and $\sigma =$ `{ojs} sdTgl`, here is a plot of the probability density:
